 ...
 *
 * @author xubin.
 * @create 2017-02-08 下午3:22
 */

@Service
@Transactional
@com.alibaba.dubbo.config.annotation.Service(interfaceClass = vip.xubin.sso.service.UserService.class, protocol = {"dubbo"}, retries = 0)
public class UserServiceImpl implements UserService {
    ...
    @Value("${ISEMAILENGAGED}")
    private String ISEMAILENGAGED;
    
    @Value("${ISMOBILEENGAGED}")
    ...
    public String validateAuthCode(String authCode, String uuid) {
        ...
        try {
            String redisAuthCode = jedisClient.get(VERIFYCODE_REDIS + uuid);
    ...
    public String register(String regName, String pwd, String pwdRepeat, String phone, String mobileCode, String uuid, String authCode, String email) {
        ...
        if (StringUtils.isNotBlank(authCode)) {
            ...
            String code = "";
            try {
                code = jedisClient.get(VERIFYCODE_REDIS + uuid);
            } catch (Exception e) {
        ...
        if (StringUtils.isNotBlank(regName)) {
            ...
            //注册成功 忽略noAuth这个词
            return "({'noAuth':'" + LOGIN_SUCCESS_URL + "?username=" + regName + "'})";
        }
        //注册失败
        return "({'error':1})";
