...
<script type="text/javascript">
    $(function () {
        ...
        var oTable1 = $('#table${random}').DataTable({
            ...
            "sAjaxSource": "/category/secondary/getTableData.action",//通过ajax实现分页的url路径。
            "aoColumns": [//初始化要显示的列
                {
                ...
                },
                {
                    ...
                    "sWidth": "70px",
                    "mRender": function (data, type, full) {//返回自定义的样式
                        var s = "<select name=''><option value='3620194' selected='selected'>select2/select2</option></select>"
                        return "<input type='text' id='cid" + dataid + "' name='cid' value='" + data + "' disabled='disabled' style='width:90px;'/>";
                ...
                },
                {
                    "mDataProp": "parentId",
                    "mRender": function (data, type, full) {//返回自定义的样式
                        return "<input type='text' id='parentId" + dataid + "' name='parentId' value='" + data + "' disabled='disabled'/>";
                ...
                },
                {
                    "mDataProp": "name",
                    "mRender": function (data, type, full) {//返回自定义的样式
                        return "<input type='text' id='name" + dataid + "' name='name' value='" + data + "' disabled='disabled' style='width:80px;'/>";
                ...
                },
                {
                    "mDataProp": "url",
                    "mRender": function (data, type, full) {//返回自定义的样式
                        return "<input type='text' id='url" + dataid + "' name='url' value='" + data + "' disabled='disabled'/>";
                ...
                },
                {
                    "mDataProp": "status",
                    "mRender": function (data, type, full) {//返回自定义的样式
                        return "<input type='text' id='status" + dataid + "' name='status' value='" + data + "' disabled='disabled'/>";
                ...
                },
                {
                    "mDataProp": "sortOrder",
                    "mRender": function (data, type, full) {//返回自定义的样式
                        return "<input type='text' id='sort" + dataid + "' name='sortOrder' value='" + data + "' disabled='disabled' style='width:40px;'/>";
                    }
                },
//                {
...
//        }
        if (typeof($("#cid" + id).attr("disabled")) == "undefined") {
            $("#cid" + id).attr("disabled", 'disabled');
        } else {
            $("#cid" + id).removeAttr("disabled");
        }
        if (typeof($("#parentId" + id).attr("disabled")) == "undefined") {
            $("#parentId" + id).attr("disabled", 'disabled');
        } else {
            $("#parentId" + id).removeAttr("disabled");
        }
        ...
        if (typeof($("#url" + id).attr("disabled")) == "undefined") {
            $("#url" + id).attr("disabled", 'disabled');
        } else {
            $("#url" + id).removeAttr("disabled");
        }
        if (typeof($("#status" + id).attr("disabled")) == "undefined") {
        ...
        <div class="col-xs-12">
            ...
            <div class="box">
                <div class="box-header">
                    <h3 class="box-title">二级商品分类列表</h3>
